#!/usr/bin/env bash
# ============================================================
#  RCM-X v2.3 — rclone + mergerfs union mount setup
#  author: greedy
...
print_header() {
    echo -e "\n${BLUE}╔══════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║${NC}  ${WHITE}RCM-X v2.3${NC} — rclone + mergerfs setup  ${BLUE}║${NC}"
    echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}\n"
}
...
check_mount_in_proc() {
  ...
  if grep -qF " $normalized_dir " /proc/mounts 2>/dev/null; then
    return 0
  fi
...
rcmx_config() {
    ...
    if ! [[ "$TRANSFERS" =~ ^[1-9][0-9]*$ ]]; then
        die "Transfers must be a positive integer, got: $TRANSFERS"
    fi
...
check_mount_conflicts() {
  ...
  local child_mount
  child_mount=$(grep -F " $MERGED_DIR/" /proc/mounts 2>/dev/null | head -1 | cut -d' ' -f2)
...
enable_services() {
  ...
  if [[ "$mergerfs_ready" == "true" ]]; then
    ...
    dim "  Checking /proc/mounts: grep -F ' $MERGED_DIR' /proc/mounts"
    grep -F " $MERGED_DIR" /proc/mounts 2>/dev/null || dim "    No match found"
